...
uint32_t *oldContext = NULL;
struct ProcessControlBlock * cop;

//this is communicating with sys_call_isr assembly in irq.s
uint32_t* sys_call(uint32_t *con){
struct Queue *readyq = get_ready_queue();
struct ProcessControlBlock *dpcb = dequeue(readyq);

    //if the current process is null, get the first one!
    if(oldContext == NULL){
        oldContext = con;
       // cop = dequeue(readyq);  nope
    }

    //put the pcb at the end of the queue
    //switch to the next
    else{
        if(params.op_code == IDLE ){
          //  remove((readyq,cop); cop isn't in the ready queue

          //1 set the state to ready
            cop->execution_state = READY;
            //might 
            enqueue_fifo(readyq,cop);
        }
    ...
    //if the ready queue isn't empty
    if(dpcb != NULL){
 
        if(dpcb->dispatching_state != SUSPENDED){
            cop = dpcb;
            cop->execution_state = RUNNING;
            return cop->top;
        }else{

            // what to do if the process is suspended
            while(1){

                //put the dequeued PCB back becuause it is suspended
                //...might be replaced with enqueue priority
                enqueue_fifo(readyq,dpcb);

                //pull out the next one
                dpcb = dequeue(readyq);
        
                //check if it is 
                if(dpcb->dispatching_state == NOT_SUSPENDED){
                    cop = dpcb;
                    cop->execution_state = RUNNING;
                    break;
                }
            ...
            return cop->top;
        }// the one dequeued was suspended

    }    
 

   // IF the ready queue is empty return oldContext
return oldContext;
}
